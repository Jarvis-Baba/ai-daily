...
Usage:
  daily-pipeline.py                           — process today's report
  daily-pipeline.py --date 2026-06-02         — process a specific date
  daily-pipeline.py --markdown <path>          — process a specific Markdown file
  daily-pipeline.py --cron                    — cron mode (minimal stdout)
"""
...
def step_content_bridge(md_path, date_str, cron=False):
    ...
    # Load Content IR for stats
    with open(ir_path) as f:
        ir = json.load(f)

    # Load Visual Plan for image count
...
def step_snapshot(date_str, vis_dir, ir_path):
    ...
    # Copy visual output to snapshot
    ...
    # Use snapshot.py's save function via subprocess
    result = subprocess.run(
        [sys.executable, snapshot_py, "save", date_str,
         ir_path, renderspec_path, html_dir, png_dir],
...
def step_diff(date_str, prev_md_path, vis_dir):
    ...
    if not os.path.exists(os.path.join(prev_snap, "manifest.json")):
        # Previous snapshot doesn't exist — create it
    ...
    # Create temp snapshot of current
    snapshot_py = os.path.join(VISUAL_COMPILER, "snapshot.py")
...
def run(date_str=None, markdown_path=None, cron=False):
    ...
    # Step 3: Diff against previous day
